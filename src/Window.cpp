...
bool bLevelButtonPressed = false;
...
PlayerData pd = PlayerData(Vector3(0, 0, 0), Vector3(0, 0, 0), Vector3(0, 0, 0), Vector3(0, 0, 0), 2, 0.0);
bool KeysDown[] = { false, false, false, false, false, false, false, false, false, false/*z*/, false /*c*/, false /*v*/ }; //wasdqe and mouse left / right and space for jumping
...
int Main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform1i(sponge_level, pd.level);
		glUniform1f(cutPlanePosition, pd.cutPlanePosition);
...
void InitGL(GLFWwindow* window)
{
	...
	sponge_level = glGetUniformLocation(shaderProgramID, "spongeLevel");

	cutPlanePosition = glGetUniformLocation(shaderProgramID, "cutPlanePosition");
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
		glfwSetWindowShouldClose(window, GL_TRUE); 		//close
	}

	if (key == GLFW_KEY_W && (action == GLFW_PRESS)) { 		//y forward
		KeysDown[0] = true;
	}
	if (key == GLFW_KEY_S && (action == GLFW_PRESS)) { 		//y back
		KeysDown[1] = true;
	}
	if (key == GLFW_KEY_A && (action == GLFW_PRESS)) {		//x left
		KeysDown[2] = true;
	}
	if (key == GLFW_KEY_D && (action == GLFW_PRESS)) {		//x right
		KeysDown[3] = true;
	}
	...
	if (key == GLFW_KEY_SPACE && (action == GLFW_PRESS)) {  // speed up
		KeysDown[6] = true;
	}
	if (key == GLFW_KEY_Z && (action == GLFW_PRESS)) { // change level
		KeysDown[7] = true;
	}
	if (key == GLFW_KEY_C && (action == GLFW_PRESS)) { // forward cut plane
		KeysDown[8] = true;
	}
	if (key == GLFW_KEY_V && (action == GLFW_PRESS)) { // backword cut plane
		KeysDown[9] = true;
	}
	

	//get the keys up (wasd)
	if (key == GLFW_KEY_W && action == GLFW_RELEASE) {		//y forward
		KeysDown[0] = false;
	}
	if (key == GLFW_KEY_S && action == GLFW_RELEASE) {		//y back
		KeysDown[1] = false;
	}
	if (key == GLFW_KEY_A && action == GLFW_RELEASE) {		//x left
		KeysDown[2] = false;
	}
	if (key == GLFW_KEY_D && action == GLFW_RELEASE) {		//x right
		KeysDown[3] = false;
	}
	if (key == GLFW_KEY_Q && action == GLFW_RELEASE) {		//up
		KeysDown[4] = false;
	}
	if (key == GLFW_KEY_E && action == GLFW_RELEASE) {		//down
		KeysDown[5] = false;
	}
	if (key == GLFW_KEY_SPACE && action == GLFW_RELEASE) {	// speed up
		KeysDown[6] = false;
	}
	if (key == GLFW_KEY_Z && (action == GLFW_RELEASE))	{
		...
		if(KeysDown[7]) {
			// change level
			KeysDown[7] = false;
			bLevelButtonPressed = true;
		}
	}
	if (key == GLFW_KEY_C && (action == GLFW_RELEASE))	{	// forward cut plane
		KeysDown[8] = false;
	}
	if (key == GLFW_KEY_V && (action == GLFW_RELEASE))	{	// backword cut plane
		KeysDown[9] = false;
	}
...
void MovePlayer(PlayerData& playerData)
{
	...
	if( bLevelButtonPressed) { // change sponge level
		pd.level += 1;
		if(pd.level == 9) pd.level = 1;
		bLevelButtonPressed = false;
	}
	if (KeysDown[8] == true) {	// forward cut plane
		pd.cutPlanePosition += 0.2;
	}
	if (KeysDown[9] == true) {	// backword cut plane
		pd.cutPlanePosition -= 0.2;
	}
}