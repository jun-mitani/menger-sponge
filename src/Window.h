...
struct PlayerData
{
	...
	Vector3 viewUp;
	int level;
	float cutPlanePosition;

	PlayerData(Vector3 Pos, Vector3 viewDir, Vector3 viewRight, Vector3 viewUp, int level, float cutPlanePosition) 
		: Pos(Pos), viewDir(viewDir), viewRight(viewRight), viewUp(viewUp),  level(level), cutPlanePosition(cutPlanePosition) {}
};
